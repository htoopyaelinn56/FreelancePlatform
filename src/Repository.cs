    ...
    internal class Repository
    {
        ...
        public (string clientName, string? freelancerName, string name, string description, decimal budget, DateTime deadline, string skills, string status, int projectId, decimal? bidAmount, int? reviewRating, string? reviewComment) getProjectDetail(int projectId)
        {
            try
            {
                ...
                string bidQuery = @"SELECT bid_amount FROM Bids WHERE project_id = @ProjectId AND status = 'approved' LIMIT 1;";
                string reviewQuery = @"SELECT rating, comment FROM Reviews WHERE project_id = @ProjectId LIMIT 1;";

                string clientName, name, description, skills, status;
                ...
                using (var cmd = new MySqlCommand(projectQuery, conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        freelancerName = reader.IsDBNull(reader.GetOrdinal("freelancerName"))
                            ? null
                            : reader.GetString("freelancerName");
                ...
                // Fetch bid amount
                using (var cmd = new MySqlCommand(bidQuery, conn))
                {
                    cmd.Parameters.AddWithValue("@ProjectId", projectId);
                    var result = cmd.ExecuteScalar();
                    if (result != null && result != DBNull.Value)
                        bidAmount = Convert.ToDecimal(result);
                }

                // Fetch review info 
                using (var cmd = new MySqlCommand(reviewQuery, conn))
                {
                    cmd.Parameters.AddWithValue("@ProjectId", projectId);
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            reviewRating = reader.IsDBNull(reader.GetOrdinal("rating")) ? null : reader.GetInt32("rating");
                            reviewComment = reader.IsDBNull(reader.GetOrdinal("comment")) ? null : reader.GetString("comment");
                        }
                    }
                }
